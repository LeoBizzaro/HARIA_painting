...
CURRENT_Z_HEIGHT = 0.350  # Starting height updated to match new minimum
...
def project_to_boundary(x, y):
    """Project a point outside the bounds to the nearest point on the boundary"""
    # Define A3 paper dimensions in meters
    a3_width = 0.210  # 297mm in meters
    a3_height = 0.297  # 420mm in meters (210mm for A4)
    ...
    # Clamp x and y to boundaries
    x_bounded = max(x_min, min(x, x_max))
    y_bounded = max(y_min, min(y, y_max))
    
    return x_bounded, y_bounded

def process_feedback(feedback):
    """Callback to update position when marker moves"""
    global last_valid_x, last_valid_y, last_update_time, last_x, last_y, filter_enabled
    ...
    if is_within_bounds(x, y):
        ...
        # Project the position to the nearest point on boundary
        bounded_x, bounded_y = project_to_boundary(x, y)
        
        # Update pose with the projected position
        update_pose(bounded_x, bounded_y, CURRENT_Z_HEIGHT)
        
        # Don't update the last_valid position as it should remain the last truly valid position
        rospy.logdebug(f"Marker outside bounds. Projected to boundary: ({bounded_x:.3f}, {bounded_y:.3f}, {CURRENT_Z_HEIGHT:.3f})")
