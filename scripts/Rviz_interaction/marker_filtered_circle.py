...
MAX_VELOCITY = 0.3  # Maximum allowed velocity in m/s
last_update_time = None
last_x = 0.5  # Initial position
last_y = 0.0
filter_enabled = True  # Flag to enable/disable filtering

# Circle drawing parameters
CIRCLE_RADIUS = 0.05  # 5cm radius by default
CIRCLE_STEPS = 24     # Number of points to make the circle
circle_center_x = 0.5  # Center of A4 by default
circle_center_y = 0.0
circle_step = 0       # Current step in the circle drawing
circle_speed = 5.0    # Seconds per circle
...
def keyboard_listener():
    ...
    rospy.loginfo("Press 'R' to draw a circle at current height")
    ...
    while not rospy.is_shutdown():
        ...
        if c == '+' or c == '=':
            ...
            # Start drawing a circle at the current position (as top point)
            rospy.loginfo(f"Drawing a circle with radius {CIRCLE_RADIUS:.3f}m starting from current position as top point")
...
def is_within_bounds(x, y):
    """Check if the position is within the A4 boundaries"""
    # Define A4 paper dimensions in meters
    a3_width = 0.210  # 210mm in meters (A4 width)
    a3_height = 0.297  # 297mm in meters (A4 height)
...
def process_feedback(feedback):
    ...
    global last_valid_x, last_valid_y, last_update_time, last_x, last_y, filter_enabled, drawing_circle
    
    # Don't process interactive feedback while drawing a circle
    if drawing_circle:
        return
...
def start_circle_drawing():
    """Setup circle drawing with current position as the top of the circle"""
    global drawing_circle, circle_center_x, circle_center_y, circle_step
    
    # Calculate the center based on current position being the top of the circle
    # If we're at the top point (0 degrees), center is CIRCLE_RADIUS below the current point
    circle_center_x = last_valid_x
    circle_center_y = last_valid_y - CIRCLE_RADIUS  # Center is RADIUS units below current position
    circle_step = 0
    drawing_circle = True
    
    rospy.loginfo(f"Circle drawing started from ({last_valid_x:.3f}, {last_valid_y:.3f}) as top point")
    rospy.loginfo(f"Circle center at ({circle_center_x:.3f}, {circle_center_y:.3f}) with radius {CIRCLE_RADIUS:.3f}m")
    
    # Start a thread for drawing the circle
    circle_thread = threading.Thread(target=draw_circle)
    circle_thread.daemon = True
    circle_thread.start()

def draw_circle():
    """Draw a circle starting from the current position as the top point"""
    global drawing_circle, circle_step
    
    rate = rospy.Rate(10)  # 10Hz update rate for smooth motion
    step_delay = circle_speed / CIRCLE_STEPS  # Time per step
    
    # We start at the top (0 degrees) and go clockwise
    start_angle = 0  # Start at the top (0 degrees)
    
    while drawing_circle and not rospy.is_shutdown() and circle_step <= CIRCLE_STEPS:
        # Calculate the angle for this step (0 to 360 degrees)
        angle = start_angle + (2 * math.pi * circle_step / CIRCLE_STEPS)
        
        # Calculate the point coordinates on the circle
        x = circle_center_x + CIRCLE_RADIUS * math.sin(angle)
        y = circle_center_y + CIRCLE_RADIUS * math.cos(angle)
        
        # Check if the point is within bounds
        if is_within_bounds(x, y):
            # Update the marker position
            update_marker_position(x, y)
            ...
            rospy.logdebug(f"Circle point: ({x:.3f}, {y:.3f}), step {circle_step+1}/{CIRCLE_STEPS}")
        else:
            rospy.logwarn(f"Circle point ({x:.3f}, {y:.3f}) is outside bounds, skipping")
        
        circle_step += 1
        
        # Sleep for the step delay
        time.sleep(step_delay)
    
    drawing_circle = False
    rospy.loginfo("Circle drawing completed")
...
if __name__ == "__main__":
    ...
    rospy.loginfo("Press 'O' to open gripper, 'C' to close gripper (if robot connected)")
    rospy.loginfo("Press 'R' to draw a circle with radius 5cm starting from current position as top point")
