...
#!/usr/bin/env python3
# Modified gripper control section of your marker_plane_keyboard_levels_2.py
...
def keyboard_listener():
    ...
    try:
        ...
        # Wait for the action servers with a timeout
    ...
    while not rospy.is_shutdown():
        ...
        if c == '+' or c == '=':  # Allow both '+' and '=' (unshifted '+')
            ...
            move_client.wait_for_result(rospy.Duration(2.0))  # Wait for completion
            gripper_closed = False
        elif (c == 'o' or c == 'O') and not gripper_available:
            rospy.loginfo("Gripper command received but no robot is connected")
                
        elif (c == 'c' or c == 'C') and gripper_available:
            rospy.loginfo("Closing gripper using GraspAction")
            goal = GraspGoal()
            goal.width = 0.01      # Target width when grasping (smaller than before)
            goal.epsilon.inner = 0.01  # Increased tolerance for inner deviation
            goal.epsilon.outer = 0.01  # Increased tolerance for outer deviation
            goal.speed = 0.05      # Reduced speed for better grasp stability
            goal.force = 20        # Increased maximum grasp force in N (was 12)
            grasp_client.send_goal(goal)
            grasp_result = grasp_client.wait_for_result(rospy.Duration(5.0))  # Wait longer for grasp
            
            if grasp_result:
                result = grasp_client.get_result()
                if result:
                    rospy.loginfo(f"Grasp completed with success: {result}")
                else:
                    rospy.logwarn("Grasp completed but no result returned")
            else:
                rospy.logwarn("Grasp did not complete in time")
                
            gripper_closed = True
        elif (c == 'c' or c == 'C') and not gripper_available:
            rospy.loginfo("Gripper command received but no robot is connected")
