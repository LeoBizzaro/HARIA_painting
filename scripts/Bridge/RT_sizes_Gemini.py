                ...
                while b"\n" in buffer:
                    ...
                    try:
                        ...
                        if color_code is not None and vr_x is None and vr_y is None:
                            handle_color_change(color_code, move_client, grasp_client, sock)
                            continue
                        ...
                        if vr_x is not None and vr_y is not None:
                            ...
                            if pencil_ready:
                                ...
                                if first_coordinate_after_idle:
                                    ...
                                    gradual_approach_from_idle(raw_x, raw_y, sock)
                                else:
...
def handle_color_change(color_code, move_client, grasp_client, sock):
    """
    ...
    if normalized_color_code in COLOR_CODE_TO_PENCIL:
        ...
        if current_pencil is not None:
            rospy.loginfo("Sending STOP message due to color change.")
            sock.sendall(b"STOP\n")
...
def gradual_approach_from_idle(target_x, target_y, sock):
    """
