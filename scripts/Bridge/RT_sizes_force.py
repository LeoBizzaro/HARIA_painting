...
MAX_Z_HEIGHT = 0.25
Z_ACTIVE_BIG = 0.113
Z_ACTIVE_SMALL = 0.130 # They start drawing at 0.128
...
PENCIL_BASE_X = 0.335        # Starting X coordinate for pencil pickup
...
for i in range(1, 21):
    ...
    if i <= 5:  # Left side, thick
        ...
        pos = {"x": PENCIL_BASE_X + (i-11)*PENCIL_X_OFFSET, "y": PENCIL_BASE_Y_LEFT - 0.107, "z": 0.135}
    elif i >= 16: # Right side, tight
        pos = {"x": PENCIL_BASE_X + (i-16)*PENCIL_X_OFFSET, "y": PENCIL_BASE_Y_RIGHT + 0.107, "z": 0.135}
...
# === FORCE MONITORING ===
FORCE_THRESHOLD = 2.0  # Force threshold in Newtons
current_z_force = 0.0
z_saturated_height = None  # Will store the saturated Z position
force_protection_active = False
...
def publish_pose(x, y, z):
    global z_saturated_height, force_protection_active
    
    # Apply force protection if active
    if force_protection_active and z_saturated_height is not None:
        # Don't allow Z to go below the saturated height
        if z < z_saturated_height:
            z = z_saturated_height
            rospy.logdebug("Z-coordinate saturated at {:.3f}m due to force protection".format(z))
...
def franka_state_callback(msg):
    """
    Callback to monitor external force on Z-axis
    """
    global current_z_force, z_saturated_height, force_protection_active, CURRENT_Z_HEIGHT
    
    # Extract Z-axis force (index 2 for Z component)
    current_z_force = msg.k_f_ext_hat_k[2]
    
    # Check if force exceeds threshold while drawing
    if pencil_ready and CURRENT_Z_HEIGHT <= Z_ACTIVE + 0.005:  # Small tolerance for drawing height
        if abs(current_z_force) > FORCE_THRESHOLD:
            if not force_protection_active:
                # First time exceeding threshold - saturate at current height
                z_saturated_height = CURRENT_Z_HEIGHT
                force_protection_active = True
                rospy.logwarn("Force protection activated! Z-force: {:.3f}N, saturating Z at {:.3f}m".format(
                    current_z_force, z_saturated_height))
        else:
            # Force is within acceptable range
            if force_protection_active:
                rospy.loginfo("Force protection deactivated. Z-force: {:.3f}N".format(current_z_force))
                force_protection_active = False
                z_saturated_height = None

def force_monitor():
    """
    Monitor and log force information periodically
    """
    rate = rospy.Rate(1)  # 1 Hz logging
    
    while not rospy.is_shutdown():
        if pencil_ready:
            if force_protection_active:
                rospy.loginfo("Force protection ACTIVE - Z-force: {:.3f}N, saturated at: {:.3f}m".format(
                    current_z_force, z_saturated_height))
            else:
                rospy.logdebug("Z-force: {:.3f}N (threshold: {:.1f}N)".format(
                    current_z_force, FORCE_THRESHOLD))
...
def main():
    global pub, gripper_available  # Add gripper_available to global declaration
    ...
    if move_client.wait_for_server(rospy.Duration(5.0)) and grasp_client.wait_for_server(rospy.Duration(5.0)):
        rospy.loginfo("Gripper servers ready. Picking up default pencil (Pencil 11 - Tight Black)...")
        gripper_available = True  # This now properly sets the global variable
        pickup_pencil_sequence(move_client, grasp_client, 11)
    else:
        rospy.logwarn("Gripper servers not available. Skipping default pencil pickup.")

    rospy.loginfo("=== FORCE PROTECTION CONFIGURATION ===")
    rospy.loginfo("Force threshold: {:.1f}N".format(FORCE_THRESHOLD))
    rospy.loginfo("Force monitoring topic: /franka_state_controller/franka_states")
    rospy.loginfo("=====================================")
    ...
    threading.Thread(target=force_monitor, daemon=True).start()
