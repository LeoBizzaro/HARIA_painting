...
def send_color_hex(hex_color_code):
    """
    Send color hex code in the format: "HEX_COLOR_CODE\n"
    """
    if client_conn:
        try:
            msg = f"{hex_color_code}\n"
            client_conn.sendall(msg.encode())
            print(f"[TCP SERVER] Inviato codice colore esadecimale: {msg.strip()}")
        except Exception as e:
            print(f"[ERRORE] Invio codice colore esadecimale fallito: {e}")
...
def on_color_button_click(hex_color_code):
    """
    Handles click on color buttons. Sends the hex color code.
    """
    send_color_hex(hex_color_code)
    print(f"Selected color: {hex_color_code}")
...
def tcp_server():
    ...
    print("[TCP SERVER] O codici colore nel formato: #RRGGBB")
...
def start_gui():
    global canvas
    root = tk.Tk()
    root.title("A4 Coordinate Server @ 60Hz - Format: x,y or #RRGGBB")
    root.geometry(f"{WIN_WIDTH}x{WIN_HEIGHT+100}") # Increased height for color buttons
    ...
    info_label = tk.Label(button_frame, 
                         text="A4 Landscape @ 60Hz: (0,0) bottom-right, (1400,1000) top-left | Format: x,y or #RRGGBB", 
                         fg="blue")
    ...
    for hex_color, _ in COLORS.items(): # Iterate over hex colors directly
        button = tk.Button(color_button_frame, 
                           bg=hex_color, 
                           width=3, 
                           height=1, 
                           command=lambda h=hex_color: on_color_button_click(h))
...
if __name__ == "__main__":
    ...
    print("Message format for colors: #RRGGBB")
