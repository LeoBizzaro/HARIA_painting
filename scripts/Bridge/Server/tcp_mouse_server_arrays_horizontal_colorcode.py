...
current_color_hex = "#16160F"  # Default color
...
def on_mouse_release(event):
    ...
    if drawing and len(current_stroke) > 0:
        ...
        stroke_colors.append(current_color_hex)
...
def on_mouse_drag(event):
    ...
    if drawing:
        ...
        if len(current_stroke) > 0:
            ...
            canvas.create_line(last_px, last_py, event.x, event.y, fill=current_color_hex, width=2)
...
def start_gui():
    ...
    # === Sezione selezione colore ===
    color_frame = tk.Frame(root)
    color_frame.pack(fill=tk.X)

    selected_color_label = tk.Label(root, text="Colore selezionato:", fg="black")
    selected_color_label.pack()

    selected_color_box = tk.Canvas(root, width=50, height=20, highlightthickness=1, highlightbackground="black")
    selected_color_box.pack()
    selected_color_box_rect = selected_color_box.create_rectangle(0, 0, 50, 20, fill=current_color_hex)

    def set_color(hex_code):
        global current_color_hex
        current_color_hex = hex_code
        selected_color_box.itemconfig(selected_color_box_rect, fill=hex_code)
        selected_color_label.config(text=f"Colore selezionato: {hex_code}")

    for hex_code in COLORS.keys():
        btn = tk.Button(color_frame, bg=hex_code, width=3, command=lambda c=hex_code: set_color(c))
