...
# === VR SAFETY PARAMETERS ===
# Maximum allowed distance between consecutive points (in meters)
MAX_JUMP_DISTANCE = 0.05  # 5cm - adjust this value based on your needs
# Minimum time between position updates to prevent spamming (in seconds)
MIN_UPDATE_INTERVAL = 1.0/120.0  # Allow up to 120Hz updates
# Number of consecutive "safe" points needed after a jump to resume normal operation
RECOVERY_POINTS_NEEDED = 3
...
WORKSPACE_X_MIN = WORKSPACE_CENTER_X - PAPER_EIGHT / 2  # FR3_X uses paper height
WORKSPACE_X_MAX = WORKSPACE_CENTER_X + PAPER_HEIGHT / 2
WORKSPACE_Y_MIN = WORKSPACE_CENTER_Y - PAPER_WIDTH / 2   # FR3_Y uses paper width  
WORKSPACE_Y_MAX = WORKSPACE_CENTER_Y + PAPER_WIDTH / 2
...
# === VR Safety State ===
last_safe_x = WORKSPACE_CENTER_X
last_safe_y = WORKSPACE_CENTER_Y
last_update_time = 0
recovery_counter = 0
safety_active = True
...
def is_safe_movement(new_x, new_y, prev_x, prev_y, max_distance):
    """
    Check if the movement from previous position to new position is safe
    Returns True if movement is within safe distance, False otherwise
    """
    distance = calculate_distance(prev_x, prev_y, new_x, new_y)
    return distance <= max_distance

def apply_vr_safety_filter(new_x, new_y):
    """
    Apply VR safety filtering to incoming coordinates
    Returns (filtered_x, filtered_y, is_accepted) tuple
    """
    global last_safe_x, last_safe_y, last_update_time, recovery_counter
    global total_filtered_points, total_processed_points, safety_active
    ...
    if current_time - last_update_time < MIN_UPDATE_INTERVAL:
        return last_safe_x, last_safe_y, False
    
    # Check if movement is safe
    if safety_active and is_safe_movement(new_x, new_y, last_safe_x, last_safe_y, MAX_JUMP_DISTANCE):
        # Safe movement - accept the new position
        last_safe_x = new_x
        last_safe_y = new_y
        ...
        distance = calculate_distance(new_x, new_y, last_safe_x, last_safe_y)
        total_filtered_points += 1
        
        # Log the filtered movement (but not too frequently to avoid spam)
        if total_filtered_points % 30 == 1:  # Log every 30th filtered point
            rospy.logwarn(f"VR Safety: Filtered unsafe jump of {distance:.4f}m "
                         f"(max: {MAX_JUMP_DISTANCE:.4f}m). "
                         f"Filtered {total_filtered_points}/{total_processed_points} points")
        
        # Keep the robot at the last safe position
        return last_safe_x, last_safe_y, False
    
    else:
        # Safety is disabled - accept all movements
        last_safe_x = new_x
        last_safe_y = new_y
        last_update_time = current_time
        return new_x, new_y, True
...
def pickup_pencil_sequence(move_client, grasp_client, pencil_number):
    ...
    global last_safe_x, last_safe_y
    ...
    if gripper_available:
        grasp_goal = GraspGoal(width=0.01, speed=0.05, force=10.0)
    ...
    # Initialize safety filter with current position
    last_safe_x = last_valid_x
    last_safe_y = last_valid_y
    
    rospy.loginfo(f"{pencil_pos['name']} pickup complete with Z-offset approach. Robot is ready for drawing with VR safety filter active.")
...
def keyboard_listener():
    ...
    global MAX_JUMP_DISTANCE, safety_active, total_filtered_points, total_processed_points
    ...
    rospy.loginfo(f"VR Safety: S = toggle safety, D = adjust max distance, I = safety info")
    rospy.loginfo(f"Current workspace: {SELECTED_PAPER} paper ({PAPER_WIDTH}x{PAPER_HEIGHT}m)")
    rospy.loginfo(f"VR Safety Filter: {'ACTIVE' if safety_active else 'DISABLED'} (max jump: {MAX_JUMP_DISTANCE*1000:.1f}mm)")
    ...
    while not rospy.is_shutdown():
        ...
        if c in ['1', '2', '3']:
            ...
            # Toggle VR safety filter
            safety_active = not safety_active
            status = "ENABLED" if safety_active else "DISABLED"
            rospy.loginfo(f"VR Safety Filter: {status}")
        elif c in ['d', 'D']:
            # Adjust maximum jump distance
            rospy.loginfo("Adjust max jump distance: 1=1cm, 2=2cm, 3=3cm, 5=5cm, 8=8cm")
            distance_key = getch()
            if distance_key == '1':
                MAX_JUMP_DISTANCE = 0.01
            elif distance_key == '2':
                MAX_JUMP_DISTANCE = 0.02
            elif distance_key == '3':
                MAX_JUMP_DISTANCE = 0.03
            elif distance_key == '5':
                MAX_JUMP_DISTANCE = 0.05
            elif distance_key == '8':
                MAX_JUMP_DISTANCE = 0.08
            rospy.loginfo(f"Max jump distance set to: {MAX_JUMP_DISTANCE*1000:.1f}mm")
        elif c in ['i', 'I']:
            # Display safety filter statistics and current pencil info
            filter_rate = (total_filtered_points / max(total_processed_points, 1)) * 100
            ...
            rospy.loginfo(f"VR Safety Status: {'ACTIVE' if safety_active else 'DISABLED'}")
            rospy.loginfo(f"Max jump distance: {MAX_JUMP_DISTANCE*1000:.1f}mm")
            rospy.loginfo(f"Total points processed: {total_processed_points}")
            rospy.loginfo(f"Points filtered: {total_filtered_points} ({filter_rate:.1f}%)")
            rospy.loginfo(f"Last safe position: ({last_safe_x:.3f}, {last_safe_y:.3f})")
            rospy.loginfo(f"====================")
...
def tcp_receiver():
    ...
    try:
        ...
        rospy.loginfo("Connected. Receiving VR coordinates in format: x,y")
        rospy.loginfo("VR coordinate system: (0,0)=bottom-right, (1k,1k)=top-left")
        rospy.loginfo(f"VR Safety Filter: {'ACTIVE' if safety_active else 'DISABLED'} (max jump: {MAX_JUMP_DISTANCE*1000:.1f}mm)")
            ...
            try:
                ...
                while b"\n" in buffer:
                    ...
                    try:
                        ...
                        if message_count % 120 == 0:  # Log every 2 seconds at 60Hz
                            rospy.loginfo(f"Raw VR data: '{text}' | pencil_ready={pencil_ready}")
                        
                        # Parse VR format: "x,y" (no parentheses)
                        if "," in text:
                            x_str, y_str = text.split(",", 1)  # Split only on first comma
                            vr_x = float(x_str.strip())
                            vr_y = float(y_str.strip())
                            
                            # Convert VR coordinates (0-1000) to normalized (0-1)
                            # VR system: (0,0)=bottom-right, (1000,1000)=top-left
                            norm_x = vr_x / 1400.0
                            norm_y = vr_y / 1000.0
                            
                            # Validate coordinate ranges
                            if not (0.0 <= norm_x <= 1.0 and 0.0 <= norm_y <= 1.0):
                                if message_count % 60 == 0:  # Don't spam warnings
                            ...
                            # VR: (1000,1000)=top-left -> Standard: (0,1)=top-left  
                            standard_x = 1.0 - norm_x  # Flip X: VR right->left becomes standard left->right
                            standard_y = norm_y        # Y is same: VR bottom->top = standard bottom->top
                            ...
                            # Apply VR safety filter
                            filtered_x, filtered_y, accepted = apply_vr_safety_filter(raw_x, raw_y)
                            
                            # Log mapping every 2 seconds and include safety info
                            if message_count % 120 == 0:
                                filter_rate = (total_filtered_points / max(total_processed_points, 1)) * 100
                                rospy.loginfo(f"VR→FR3: VR({vr_x},{vr_y}) -> std({standard_x:.3f},{standard_y:.3f}) -> "
                                            f"raw FR3({raw_x:.3f},{raw_y:.3f}) -> filtered FR3({filtered_x:.3f},{filtered_y:.3f}) "
                                            f"[Safety: {filter_rate:.1f}% filtered] | pencil_ready={pencil_ready}")
                            
                            # Update position only if accepted or if it's a filtered but valid position
                            last_valid_x = filtered_x
                            last_valid_y = filtered_y
                            last_tcp_time = time.time()
                            
                            # SIMPLIFIED: Robot moves if pencil_ready is True (no additional checks)
                            if pencil_ready:
                                publish_pose(filtered_x, filtered_y, Z_ACTIVE)
                            else:
                                # Log when coordinates are received but pencil is not ready
                                if message_count % 60 == 0:  # Don't spam this message
                                    rospy.loginfo(f"Coordinates received but pencil not ready. Use keys 1/2/3 to select a pencil first.")
                        else:
                            rospy.logwarn(f"No comma found in VR data: '{text}'")
                            error_count += 1
                            
                    except ValueError as e:
                        rospy.logwarn(f"Value parsing error: {e} for line: '{text}'")
                        error_count += 1
                    except Exception as e:
                        rospy.logwarn(f"Unexpected parsing error: {e} for line: '{text}'")
                        error_count += 1
                ...
                if current_time - last_debug_time >= 2.0:
                    filter_rate = (total_filtered_points / max(total_processed_points, 1)) * 100
                    rospy.loginfo(f"TCP Stats: {message_count} messages, {error_count} errors, "
                                f"pencil_ready={pencil_ready}, safety_filter={filter_rate:.1f}% filtered")
...
def main():
    global pub
    rospy.init_node("tcp_drawing_homepos_filtered")
    ...
    rospy.loginfo(f"=== VR SAFETY CONFIGURATION ===")
    rospy.loginfo(f"Safety filter: {'ENABLED' if safety_active else 'DISABLED'}")
    rospy.loginfo(f"Max jump distance: {MAX_JUMP_DISTANCE*1000:.1f}mm")
    rospy.loginfo(f"Min update interval: {MIN_UPDATE_INTERVAL*1000:.1f}ms")
    rospy.loginfo(f"===============================")
