...
CartesianImpedanceController::CartesianImpedanceController(ros::NodeHandle& nh) : nh_(nh), has_pose_(false) {
    ...
    K_.setIdentity();
    K_.topLeftCorner(3, 3) << 250, 0, 0, 0, 200, 0, 0, 0, 300; // Increased Z stiffness
    K_.bottomRightCorner(3, 3) << 20, 0, 0, 0, 50, 0, 0, 0, 25; // Increased Y rotation stiffness

    D_.setIdentity();
    D_.topLeftCorner(3, 3) << 25, 0, 0, 0, 20, 0, 0, 0, 25; // Increased Z damping
    D_.bottomRightCorner(3, 3) << 5, 0, 0, 0, 10, 0, 0, 0, 10; // Increased Y rotation damping
