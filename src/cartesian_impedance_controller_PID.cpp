...
CartesianImpedanceController::CartesianImpedanceController(ros::NodeHandle& nh) : nh_(nh), has_pose_(false) {
    ...
    Ki_.setZero();
    Ki_.diagonal() << 0.1, 0.1, 0.1, 0.05, 0.05, 0.05;
    dt_ = 0.001;

    // Set impedance parameters (Stiffness and Damping)
    // K_.setIdentity();
    // K_.topLeftCorner(3, 3) << 250, 0, 0, 0, 200, 0, 0, 0, 200; // Translational stiffness
    // K_.bottomRightCorner(3, 3) << 20, 0, 0, 0, 40, 0, 0, 0, 20;  // Rotational stiffness

    // D_.setIdentity();
    // D_.topLeftCorner(3, 3) << 25, 0, 0, 0, 20, 0, 0, 0, 20; // Translational damping
    // D_.bottomRightCorner(3, 3) << 5, 0, 0, 0, 8, 0, 0, 0, 5;   // Rotational damping

    K_.setIdentity();
    K_.topLeftCorner(3, 3) << 200, 0, 0, 0, 180, 0, 0, 0, 160; // Increased Z stiffness
    K_.bottomRightCorner(3, 3) << 15, 0, 0, 0, 35, 0, 0, 0, 20; // Increased Y rotation stiffness

    D_.setIdentity();
    D_.topLeftCorner(3, 3) << 25, 0, 0, 0, 20, 0, 0, 0, 20; // Increased Z damping
    D_.bottomRightCorner(3, 3) << 5, 0, 0, 0, 8, 0, 0, 0, 5; // Increased Y rotation damping
...
void CartesianImpedanceController::run() {
    ...
    while (ros::ok()) {
        if (has_pose_) {
            ...
            // 2. Update integral
            error_integral_ += error_x * dt_;

            // 3. Anti-windup protection
            double max_integral = 0.05; // Adjust based on your application
            for (int i = 0; i < 6; ++i) {
                error_integral_(i) = std::max(std::min(error_integral_(i), max_integral), -max_integral);
}
            ...
            Eigen::Matrix<double, 6, 1> desired_force = K_ * error_x - D_ * dx + Ki_ * error_integral_;
