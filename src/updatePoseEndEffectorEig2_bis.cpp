...
public:
    UpdatePoseEndEffector()
    {
        ...
        nh.param("speed_constant", speed_constant, 0.6);  // Increased from 0.3 for faster movement
    ...
    void frankaStateMessageReceived(const franka_msgs::FrankaState& robot_state) {
        ...
        nh.param("speed_constant", speed_constant, 0.6);
    ...
    // Apply joint velocity filtering instead of Cartesian pose filtering
...
if (pub_vel.getNumSubscribers() > 0 && !invalid) {
            ...
            if (adaptive_filtering) {
                ...
                nh.param("max_filter_alpha", max_filter_alpha, 0.7);
                nh.param("filter_transition_distance", filter_transition_distance, 0.05);
